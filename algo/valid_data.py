...
def get_all_combinations_df(result):
    ...
    # все комбинации групп и аудиторий, но чтобы аудитория вмещала класс
...
def get_ways_for_timeslots(result):
    ...
    return all_combs, {s: np.concatenate([np.full(5, -1), arr]) for s, arr in slots.items()} # возможность окна
