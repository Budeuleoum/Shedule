...
from geneticalgorithm2 import plot_pop_scores
...
class Optimizator:
    ...
    #для функции метрики, чтобы она оптимально читалась (в плане времени), чтобы кэшировать некоторые результаты
    def set_evaluators(self):

        #сохраняю некоторые вещи заранее в локальных переменных
        needed_df = self.combinations.loc[:, ['subject', 'group']].values 
        ...
        group_keys = {group.id: i for i, group in enumerate(all_groups)} 
        last_groups = [i for i, g in enumerate(all_groups) if g.last] 
    ...
    def find_solution(self, hash):
        ...
        #определяю функцию, которой буду оптимизировать
        func = lambda arr: self.eval(self.indexes2rows(arr))

        #сама модель алгоритма
        model = ga(func, 
        ...
        #покоординатный спуск (после 1-го прохода) 
        ...
        #маленькая оптимизация после 50 поколений стагнации
        ...
        #применение оптимизации после 50 поколений стагнации
        ...
        plot_for_gif(model.output_dict['last_generation']['scores'], title = 'Значение популяции после первой оптимизации ГА', save_as= f'{hash}_pop_after_GA.png')
        ...
        plot_for_gif(gen['scores'], title = 'Поколение после удаления дубликатов', save_as= f'{hash}_pop_after_remove_dups.png')

        best_val = gen['scores'].min()

        print(f"--------------> Восхождение по холму:")
        ...
        for i in range(5):
            ...
            print(f"---> {t} оптимизируется в {gen['scores'][i]}")

        # второй раз ГА
        if best_val > gen['scores'][:10].min():
            
            plot_for_gif(gen['scores'], title = 'Значение популяции после восхождения по холму', save_as= f'{hash}_pop_after_HC.png')
            
            print('--------------> Вторая оптимизация ГА')
            model.run(
                no_plot = False, 
                disable_progress_bar = False,
                disable_printing = False,
    ...
    def save_results(self, file_name = None):
        ...
        df = self.combinations.iloc[vec[mask], :].copy() # вся нужная инфа про слот -- учитель, предмет, кабинет, класс
        ...
        last_groups = [i for i, g in enumerate(all_groups) if g.last] # считаю по ним метрики
        timeslot2day = np.zeros(len(self.slots), dtype = int) # соответствие между номером слота и днём
        for i in range(1, 7):
            timeslot2day[(i-1)*7:i*7] = i
        slot_indexes = np.arange(len(self.slots))
